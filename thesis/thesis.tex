...
\forall a, b \in A : a \leq b \land b \leq a \Rightarrow a = b \\
\forall a, b, c \in A : a \leq b \land b \leq c \Rightarrow a \leq c \\
\end{align}

These rules are also known as \textit{reflexivity} (1),
\textit{antisymmetry} (2) and \textit{transitivity} (3).
...
\item element $l$ in $A$ is an \textit{lower bound} of $S \subseteq A$ if
...
To conclude, the abstract interpretation attempts to achieve fixpoint of
...
representing strings \texttt{""} can be found in figure \ref{fig:trie}.
...
\end{tikzpicture}
\caption{Trie representing strings \texttt{"great"}, \texttt{"growth"} and \texttt{"good"}}
\label{fig:trie}
\end{figure}
