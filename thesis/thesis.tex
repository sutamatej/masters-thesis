...
abstract strings so that they provide a useful information about file
paths.
...
Partial order on this domain is defined as:
...
The least upper bound operator is defined as:

\begin{align*}
\sqcap (\mathsf{S_1}, \mathsf{S_2}) =
\begin{cases}
\mathsf{S} & \text{where } \mathsf{S} = prefix(\mathsf{S_1}, \mathsf{S_2}) \\
\top       & \text{otherwise}
\end{cases}
\end{align*}

where function $prefix(\mathsf{S_1}, \mathsf{S_2})$ returns longest common
prefix of strings $\mathsf{S_1}$ and $\mathsf{S_2}$.

Simply put, the least upper bound of two strings $\mathsf{S_1}$ and
$\mathsf{S_2}$ is the longest common prefix of these strings. If there isn't
one, the upper bound is the top element of the lattice $\top$.

In a similar fashion, the greater lower bound operator is defined by:
