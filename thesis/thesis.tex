...
have different properties. TODO
...
Following the definition of abstract domain from section TODO set, top, bottom
\ref{sec:abstractinterpretation} these are the operations that
need to be defined for every suggested domain:

\begin{itemize}

\item order for elements $\leq$

\item least upper bound $\sqcap$

\item greater lower bound $\sqcup$
...
specific operations -- concatenation, substring, why these two??
...
Partial ordering on this domain is defined as:
...
The semantics for \textit{concatenation} operation is defined as:
