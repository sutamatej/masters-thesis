...
\node[block] (init) at (0, 2) {\texttt{x = 1, i = 0}};
\node[block] (loop) at (0, 0) {\texttt{x = x * i, i = i + 1}};
\node[block] (cond) at (0, 1) {\texttt{i < 5}};
\node[block] (end) at (0, -1) {\texttt{return x}}

\draw [->] (init) -- (loop);
\draw [->] (loop) -- (cond);
\draw [->] (cond) -- (loop) node [midway, fill=white] {yes};
...
the concrete domain is a complete lattice $L$ \cite{Constantini11-1}:
