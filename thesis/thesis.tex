...
\texttt{Python}, \texttt{Ruby} or \texttt{Perl}. They are used to express
values, arguments, commands or even whole pieces of code. Strings are
frequently used during generating an output such as \texttt{XML}
file or processing an input in text form. The format of the processed
string is often verified by another part of the system which takes this
string as an input, for instance the format of a \texttt{SQL} query is
verified by SQL server processor which can cause unexpected errors.
Moreover, a user might supply a malicious input when deliberately trying to
cause similar effect. Therefore, it is useful to know how string is
formatted and manipulated during the execution of a program.

A string over an alphabet is defined as a finite sequence of symbols,
where an alphabet is a nonempty finite set and members of alphabet are
symbols. Traditionally, an alphabet is denoted as $\varsum$.

example of an alphabet and a string
...
the code by pair of \texttt{"} characters. Internally, they represent a
chunk of memory structurally similar to array where every member of the
array has a data type of \texttt{char}. In fact, string is a specific type
of array. Generally, an array can contain any type of data (TODO
marginalia) given all the array items have the same one and for string
the data type happens to be \texttt{char}. The end of the string is
determined by a special character \texttt{'\0'}.

Obviously, strings and arrays are related to each other quite tightly.
Both of them sets of values sequentially stored in memory. Nonetheless,
there is a difference in the information value they represent. For example,
in an array of integers the information value is uniformly distributed --
every number in the array carries the same weight. On the other hand,
symbols of string do not carry a lot of information just by themselves.
