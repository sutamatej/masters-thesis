...
suffice. This function have been chosen because it is one of the few that
modify string. It is denoted as \textit{concat}.
...
The implementation has been checked for memory leaks with \texttt{valgrind}
...
and \texttt{==}. Table \ref{tab:prefixorder} shows all possible cases that
can occur for these operators.
...
\begin{tabular}{c|c|c|c}
 x & y & \texttt{<} & \texttt{==} \\
\hline
$\top$         & $\top$         & \texttt{false} & $\top$ \\
$\top$         & \texttt{"str"} & \texttt{false} & \texttt{"str"} \\
$\top$         & $\bot$         & \texttt{false} & $\bot$ \\
\texttt{"str"} & $\top$         & \texttt{true}  & \texttt{"str"} \\
\texttt{"str"} & \texttt{"str"} & \texttt{true} if y is prefix of x &  \\
\texttt{"str"} & $\bot$         & \texttt{false} & $\bot$ \\
$\bot$         & $\top$         & \texttt{true}  & $\bot$ \\
$\bot$         & \texttt{"str"} & \texttt{true}  & $\bot$ \\
$\bot$         & $\bot$         & \texttt{false} & $\bot$
\end{tabular}
\caption{Results of \texttt{<} and \texttt{==} operators}
\label{tab:prefixorder}
\end{table}

The return values of \texttt{join()} and \texttt{meet()} methods can be
observed in table \ref{tab:prefixjoinmeet}. The function $\textit{prefix}(x, y)$
from definition \ref{def:prefixjoin} is implemented as a private function
\texttt{commonPrefix()} which takes two strings as arguments and returns their
longest common prefix.

\begin{table}[ht]
\centering
\begin{tabular}{c|c|c|c}
 x & y & \texttt{join()} & \texttt{meet()} \\
\hline
$\top$          & $\top$          & $\top$          & $\top$ \\
$\top$          & \texttt{"text"} & $\top$          & \texttt{"text"} \\
$\top$          & $\bot$          & $\top$          & $\bot$ \\
\texttt{"text"} & $\top$          & $\top$          & \texttt{"text"} \\
\texttt{"text"} & \texttt{"text"} & see def. \ref{def:prefixjoin} & see def. \ref{def:prefixmeet} \\
\texttt{"text"} & $\bot$          & \texttt{"text"} & $\bot$ \\
$\bot$          & $\top$          & $\top$          & $\bot$ \\
$\bot$          & \texttt{"text"} & \texttt{"text"} & $\bot$ \\
$\bot$          & $\bot$          & $\bot$          & $\bot$
\end{tabular}
\caption{Results of \texttt{join()} and \texttt{meet()} methods}
\label{tab:prefixjoinmeet}
\end{table}
...
Designed string abstract domains are implemented in following files:
